    ...
    public void onEntityDeath(final @NotNull EntityDeathEvent event) {
        ...
        if (plugin.getHeadsLoader().getEntries().containsKey(key) == true) {
            ...
            final @Nullable String variant = switch (event.getEntity()) {
                ...
                case Cow cow -> (Bukkit.getUnsafe().getProtocolVersion() >= 770 == true) ? cow.getVariant().getKey().asString() : null;
                case Pig pig -> (Bukkit.getUnsafe().getProtocolVersion() >= 770 == true) ? pig.getVariant().getKey().asString() : null;
                case Chicken chicken -> (Bukkit.getUnsafe().getProtocolVersion() >= 770 == true) ? chicken.getVariant().getKey().asString() : null;
                default -> null;
            };
